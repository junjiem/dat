 ...
 */
public class Configuration implements ReadableConfig, WritableConfig {
    ...
    public boolean contains(ConfigOption<?> configOption) {
        synchronized (this.confData) {
            final BiFunction<String, Boolean, Optional<Boolean>> applier =
                    (key, canBePrefixMap) -> {
                        if (canBePrefixMap && ConfigurationUtils.containsPrefixMap(this.confData, key)
     ...
     * sure there will be at least one value available. Otherwise, a NPE will be thrown when the value is used.
     *
    ...
    public <T> boolean removeConfig(ConfigOption<T> configOption) {
        synchronized (this.confData) {
            final BiFunction<String, Boolean, Optional<Boolean>> applier =
                    (key, canBePrefixMap) -> {
                        if (canBePrefixMap && ConfigurationUtils.removePrefixMap(this.confData, key)
