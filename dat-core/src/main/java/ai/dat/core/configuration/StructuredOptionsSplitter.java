...
class StructuredOptionsSplitter {
    ...
    private static List<String> processTokens(List<Token> tokens) {
        ...
        for (int i = 0; i < tokens.size(); i++) {
            ...
            switch (token.tokenType()) {
                ...
                case SINGLE_QUOTED:
                    if (i + 1 < tokens.size()
                            && tokens.get(i + 1).tokenType() != TokenType.DELIMITER) {
                        int illegalPosition = tokens.get(i + 1).position() - 1;
                        throw new IllegalArgumentException(
                                "Could not split string. Illegal quoting at position: "
                                        + illegalPosition);
                    }
                    splits.add(token.string());
                    break;
                case UNQUOTED:
                    splits.add(token.string());
                    break;
    ...
    private record Token(TokenType tokenType, String string, int position) {
    }
