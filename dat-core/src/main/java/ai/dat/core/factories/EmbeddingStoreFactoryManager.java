...
public class EmbeddingStoreFactoryManager {
    ...
    private static final Map<String, EmbeddingStoreFactory> factories = new HashMap<>();

    static {
        for (EmbeddingStoreFactory factory : ServiceHelper.loadFactories(EmbeddingStoreFactory.class)) {
            String identifier = factory.factoryIdentifier();
            Preconditions.checkArgument(
                    PATTERN.matcher(identifier).matches(),
                    "Invalid embedding store factory identifier: '%s'. " +
                            "Only letters, numbers, underscores (_), and hyphens (-) are allowed.",
                    identifier
            );
            Preconditions.checkArgument(!factories.containsKey(identifier),
                    "There is already a embedding store factory identifier with the same name:"
                            + identifier);
    ...
    public static EmbeddingStoreFactory getFactory(String identifier) {
        EmbeddingStoreFactory factory = factories.get(identifier);
        Preconditions.checkNotNull(factory,
                "Unsupported embedding store factory identifier: " + identifier +
